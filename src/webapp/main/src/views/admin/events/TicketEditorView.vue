...
<script>
    ...
    export default {
        ...
        methods: {
            saveTemplate() {
                const saving = this.$refs.notification.createNotification( 'Saving...', 5, 'progress', 'normal' );
                let fetchOptions = {
                    method: 'post',
                ...
                fetch( '/admin/api/saveTickets', fetchOptions ).then( res => {
                    if ( res.status === 200 ) {
                        res.text().then( () => {
                            this.$refs.notification.cancelNotification( saving );
        ...
        created() {
            setTimeout( () => {
                ...
                fetch( '/admin/API/getTicketTemplate?ticket=' + sessionStorage.getItem( 'selectedTicket' ) ).then( res => {
                    const template = {
                    ...
                    if ( res.status === 200 ) {
                        res.json().then( data => {
                            if ( Object.keys( data ) ) {
                                this.designer = new Designer( { domContainer, data } );
                            } else {
